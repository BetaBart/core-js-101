...
function isCreditCardNumber(ccn) {
  ...
  return arr.reduce((accum, curr) => +accum + +curr) % 10 === 0;
}
...
function getDigitalRoot(num) {
  ...
  return arr.length > 1 ? getDigitalRoot(arr.reduce((accum, curr) => +accum + +curr)) : num;
}
...
function getCommonDirectoryPath(pathes) {
  const aa = pathes.map((el) => el.split('/'));
  let res = aa[0];
  for (let i = 1; i < pathes.length; i += 1) {
    res = res.filter((el, ind) => aa[i][ind] === el);
  }
  return res.length > 0 ? `${res.join('/')}/` : res.join('');
}
...
function getMatrixProduct(m1, m2) {
  ...
  const m2colums = m2[0].length;
  const m2rows = m2.length;
  const m12 = [];
  for (let i = 0; i < m1rows; i += 1) {
    m12[i] = [];
  }
  for (let t = 0; t < m2colums; t += 1) {
    for (let j = 0; j < m1rows; j += 1) {
      let res = 0;
      for (let k = 0; k < m2rows; k += 1) {
        res += m1[j][k] * m2[k][t];
      }
      m12[j][t] = res;
    }
